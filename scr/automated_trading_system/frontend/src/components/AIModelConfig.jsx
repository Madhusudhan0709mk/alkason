  ...
  const fetchAIModels = async () => {
    try {
      ...
      if (Array.isArray(response.data)) {
        ...
        setAIModels([]); // Set to an empty array in case of an error
      }
    } catch (error) {
    ...
    <div className="ai-model-config">
      ...
      <form onSubmit={handleSubmit}>
        ...
        {newModel.modelType === 'sentiment' && (
          <input
            ...
            placeholder="Sentiment Threshold"
          />
        )}
      ...
      <ul>
        {Array.isArray(aiModels) &&
          aiModels.map((model) => (
            <li key={model.name}>
              {model.name} - Provider: {model.provider}, Type: {model.modelType}, Active: {model.isActive ? 'Yes' : 'No'}
              <button
                onClick={() => handleUpdate({ ...model, isActive: !model.isActive })}
              >
